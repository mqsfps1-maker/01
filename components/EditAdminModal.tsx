...
const EditAdminModal: React.FC<EditAdminModalProps> = ({ isOpen, onClose, userToEdit, onConfirmUpdate, generalSettings }) => {
    ...
    const handleRoleChange = (newRole: UserRole) => {
        ...
        // FIX: Updated UserRole to align with the database schema
        if (newRole === 'FUNCIONARIO' || (userToEdit && userToEdit.role === 'FUNCIONARIO')) {
            setEmail('');
            setPassword('');
            setConfirmPassword('');
        }
    ...
    const handleConfirm = async () => {
        setError('');
        const isLoginRequired = role === 'CLIENTE_GERENTE' || role === 'DONO_SAAS';
    ...
    // FIX: Updated UserRole to align with the database schema
    const isLoginEnabled = role === 'CLIENTE_GERENTE' || role === 'DONO_SAAS';
                ...
                <div className="space-y-4">
                    ...
                    <div>
                        ...
                        <select value={role} onChange={e => handleRoleChange(e.target.value as UserRole)} className="mt-1 p-2 w-full border border-[var(--color-border)] bg-[var(--color-surface)] rounded-md text-sm focus:ring-blue-500 focus:border-blue-500">
                            {/* FIX: Updated UserRole values to align with the database schema */}
                            <option value="FUNCIONARIO">Funcionário (Sem Login)</option>
                            <option value="CLIENTE_GERENTE">Gerente (Com Login)</option>
                            {userToEdit.role === 'DONO_SAAS' && <option value="DONO_SAAS">Dono do SaaS</option>}
                        </select>
                    </div>
                    <div>
