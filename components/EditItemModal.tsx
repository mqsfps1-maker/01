...
interface EditItemModalProps {
    ...
    item: StockItem;
    currentUser: User;
    onConfirm: (itemId: string, updates: Partial<Pick<StockItem, 'name' | 'min_qty' | 'category' | 'color' | 'product_type' | 'substitute_product_code'>>) => Promise<boolean>;
...
const EditItemModal: React.FC<EditItemModalProps> = ({ isOpen, onClose, item, currentUser, onConfirm, generalSettings, products }) => {
    ...
    const handleConfirm = async () => {
        ...
        const updates: Partial<Pick<StockItem, 'name' | 'min_qty' | 'category' | 'color' | 'product_type' | 'substitute_product_code'>> = {
            name: name.trim(),
        };
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-lg shadow-2xl p-6 w-full max-w-lg">
                ...
                <div className="space-y-4">
                    ...
                    {(item.kind === 'PRODUTO' || item.kind === 'INSUMO') && (
                         <div>
