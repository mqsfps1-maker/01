...
const Header: React.FC<HeaderProps> = ({ filters, onFilterChange, onSettingsClick, generalSettings }) => {
  ...
  const CanalButton: React.FC<{canal: Canal, label: string}> = ({ canal, label }) => {
    let buttonClasses = 'text-[var(--color-text-secondary)] hover:bg-[var(--color-surface-secondary)]';
    if (filters.canal === canal) {
        if (canal === 'ML') {
            buttonClasses = 'bg-[var(--color-ml)] text-[var(--color-ml-text)] font-semibold';
        } else if (canal === 'SHOPEE') {
            buttonClasses = 'bg-[var(--color-shopee)] text-[var(--color-shopee-text)] font-semibold';
        } else {
            buttonClasses = 'bg-[var(--color-primary)] text-[var(--color-primary-text)] font-semibold';
        }
    }
    return (
      <button
        onClick={() => setCanal(canal)}
        className={`px-3 py-1 text-sm rounded-md ${buttonClasses}`}
      >
        {label}
      </button>
    );
