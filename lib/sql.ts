...
// SQL_FULL_DB_SYNC: created by previous scripts
export const FULL_DB_SYNC_SQL = `
...
-- (omitted here for brevity; use full SQL file content in production)
-- Use the file in the repo: SQL_FULL_DB_SYNC.sql
`;

export const ERP_TABLES_SQL = `
-- SQL_CREATE_ERP_TABLES.sql
-- Cria as tabelas ERP necessárias pela aplicação (idempotente)
-- Execute no Supabase SQL Editor (copiar + RUN).
\n
-- (omitted: full table creation SQL; use file SQL_CREATE_ERP_TABLES.sql in repo)
`;

export const AUTOCREATE_ERP_TABLES_SQL = `
