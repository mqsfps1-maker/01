...
}> = ({ title, description, rules, ruleType, onAdd, onRemove, stockItems }) => {
    const [from, setFrom] = useState(1);
    const [to, setTo] = useState(1);
    const [stockItemCode, setStockItemCode] = useState('');
    const [quantity, setQuantity] = useState(1);
    const insumos = stockItems.filter(i => i.kind === 'INSUMO');
        ...
        return (
            <div>
                <label className="text-sm font-medium text-[var(--color-text-secondary)]">{label}</label>
                 {headers.length > 0 ? (
                    <select name={`${platform}.${field}`} value={settings.importer[platform][field as keyof typeof settings.importer[typeof platform]] as string} onChange={e => {
                        const {name, value} = e.target;
                        const [_, child] = name.split('.');
                        handleNestedInputChange('importer', `${platform}.${child}`, value);
                    }} className="mt-1 w-full p-2 border border-[var(--color-border)] bg-[var(--color-surface)] rounded-md">
                        <option value="">Selecione a coluna...</option>
                        {headers.map(h => <option key={h} value={h}>{h}</option>)}
                    </select>
                ) : (
                    <input type="text" name={`${platform}.${field}`} value={settings.importer[platform][field as keyof typeof settings.importer[typeof platform]] as string} onChange={e => {
                        const {name, value} = e.target;
                        const [_, child] = name.split('.');
                        handleNestedInputChange('importer', `${platform}.${child}`, value);
                    }} className="mt-1 w-full p-2 border border-[var(--color-border)] bg-[var(--color-surface)] rounded-md" placeholder="Digite o nome do cabeçalho"/>
                )}
            </div>
        );
    };
        ...
        <div className="space-y-8">
            <div className="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p className="text-sm text-[var(--color-text-secondary)] mt-1">Gerencie a conexão, estrutura e dados do seu sistema.</p>
                </div>
            </div>
                 ...
                 <div className="space-y-8">
                    <Section title="Configurações da Aplicação" icon={<Settings2 size={20} className="mr-2 text-blue-600" />}>
                        <div>
                            ...
                            <div className="mt-1 flex rounded-md shadow-sm">
                                ...
                                <input
                                    ...
                                    id="companyName"
                                    name="companyName"
                                    value={settings.companyName.replace(/^TagsFlow - /, '')}
                                    onChange={(e) => setSettings(prev => ({ ...prev, companyName: `TagsFlow - ${e.target.value}` }))}
                        ...
                        <div className="border-t border-[var(--color-border)] pt-4">
                            ...
                            />
                             <ExpeditionRuleEditor
                                title={`Regras de Embalagem (${settings.productTypeNames.miudos})`}
                                description={`Defina qual embalagem usar para pedidos que contêm APENAS ${settings.productTypeNames.miudos}.`}
