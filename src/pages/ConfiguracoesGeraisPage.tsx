...
const ConfiguracoesGeraisPage: React.FC<ConfiguracoesGeraisPageProps> = (props) => {
    const {
    ...
    const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
        ...
        // Regra de Negócio: Nome da empresa sempre começa com "TagsFlow - "
        if (name === 'companyName') {
            let newValue = value;
            if (!newValue.startsWith('TagsFlow - ')) {
                // Se o usuário tentar apagar, forçamos o prefixo
                newValue = 'TagsFlow - ' + newValue.replace(/^TagsFlow - /, '');
            }
    ...
    const handleUpdateRule = (type: RuleType, id: string, changes: Partial<ExpeditionRule>) => {
        setSettings(prev => ({
            ...prev,
            expeditionRules: {
                ...prev.expeditionRules,
                [type]: (prev.expeditionRules?.[type] || []).map(r => r.id === id ? { ...r, ...changes } : r)
            }
        }));
    };
    ...
    const ExpeditionRuleEditor: React.FC<{
        ...
        onUpdate?: (type: RuleType, id: string, changes: Partial<ExpeditionRule>) => void;
        ...
        const handleAddClick = () => {
            if (from > 0 && to >= from && stockItemCode && quantity > 0) {
                onAdd(ruleType, { from, to, stockItemCode, quantity, labelOverride, productType });
                // Reset form
                setFrom(to + 1);
                setTo(to + 1);
                setLabelOverride('');
        ...
        return (
            <div className="border-t pt-4">
                ...
                <div className="flex items-end gap-2 p-2 bg-gray-50 border rounded-md flex-wrap">
                    ...
                    <div className="flex-grow min-w-[150px]">
                        <label className="text-xs">Texto entre parênteses (opcional)</label>
                        <input type="text" value={labelOverride} onChange={e => setLabelOverride(e.target.value)} className="w-full p-1 border rounded-md" placeholder="Ex: (Opcional)" />
                    </div>
                ...
                {isEditOpen && editingRule && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
                        <div className="bg-white rounded-lg shadow-xl p-6 w-full max-w-md">
                            <h3 className="text-lg font-bold mb-4">Editar Regra</h3>
                            <div className="space-y-3">
                                <div>
                                    <label className="text-sm">Texto entre parênteses</label>
                                    <input className="w-full p-2 border rounded mt-1" value={editingRule.labelOverride || ''} onChange={e => setEditingRule(r => r ? {...r, labelOverride: e.target.value} : r)} />
                                </div>
                                <div>
                            ...
                            <div className="mt-4 flex justify-end gap-2">
                                <button onClick={() => { setIsEditOpen(false); setEditingRule(null); }} className="px-3 py-2 rounded border">Cancelar</button>
                                <button onClick={() => {
                                    if (editingRule && onUpdate) {
                                        onUpdate(ruleType, editingRule.id, { labelOverride: editingRule.labelOverride || undefined, productType: editingRule.productType });
                                    }
                                    setIsEditOpen(false);
                                    setEditingRule(null);
                                }} className="px-3 py-2 bg-blue-600 text-white rounded">Salvar</button>
                            </div>
                        </div>
                    </div>
                )}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
                 <div className="space-y-8">
                    <Section title="Configurações da Aplicação" icon={<Settings2 size={20} className="mr-2 text-blue-600" />}>
                        ...
                        <div className="border-t border-[var(--color-border)] pt-4">
                            <h3 className="text-base font-semibold text-[var(--color-text-primary)]">Regras de Expedição</h3>
                                 <ExpeditionRuleEditor
                                    title="Regras de Embalagem"
                                    description={`Defina qual embalagem usar com base na quantidade de itens e no tipo de produto cadastrado.`}
...
export default ConfiguracoesGeraisPage;
